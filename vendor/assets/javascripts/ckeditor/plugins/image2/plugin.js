...
( function() {
	...
	function widgetDef( editor ) {
		...
		return {
			...
			styleableElements: 'img figure figcaption',
	...
	function setDimensions( widget ) {
		...
		if (caption) {
			caption.setStyle("width", parseInt(data.width) + "px");
		}
	}
		...
		resizer.on( 'mousedown', function( evt ) {
			...
			function onMouseMove( evt ) {
				...
				if ( newWidth >= 15 && newHeight >= 15 ) {
					...
					caption.setStyle("width", newWidth+'px');
					updateData = true;
				} else
					updateData = false;
			}
	...
	function getWidgetFeatures( editor ) {
		var alignClasses = editor.config.image2_alignClasses,
			features = {
				...
				caption: {
					requiredContent: 'figcaption[style]'
				}
			};
